...
public class ImpactAlexaHandler extends BattleshipHandler {

    private static Logger log = getLogger(ImpactAlexaHandler.class);

    @Override
    public boolean validarHandle(HandlerInput handlerInput, IntentRequest intentRequest) {
        return handlerInput.matches(requestType(IntentRequest.class).and(intentName(i18n(locale, "impactAlexaIntent"))));
    }

    @Override
    public Optional<Response> procesarHandle(HandlerInput handlerInput, IntentRequest intentRequest) {
        log.debug("Executing ImpactAlexaHandler");
        ...
        new BattleshipGame().saveImpactOnPlayerBoard(game, alexaAtaqueFila, alexaAtaqueColumna);
        log.info("Player board updated with impact");
        ...
        SendCommandDirective payload = new SendCommandDirective("command","alexa_impact");
        int randomSpeech = new Random().nextInt(6) + 1;
        return handlerInput.getResponseBuilder()
                .withSpeech(i18n(locale, "impactItIsYourTurnSpeechText" + randomSpeech))
